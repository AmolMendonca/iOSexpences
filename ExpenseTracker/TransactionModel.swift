...
struct Transaction: Identifiable, Decodable, Hashable {
    ...
    // Function to parse JSON and convert date
    func parseTransaction(jsonData: Data) -> String? {
        let decoder = JSONDecoder()
        if let transaction = try? decoder.decode(Transaction.self, from: jsonData) {
            return formattedDate(dateString: transaction.date)
        }
        return nil
    }

    // Helper function to convert date string to "Month Year" format
